...
public class TagAliasOperatorHelper {
                    ...
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                       LogUtil.log(TAG,"on delay time");
                        ...
                        if(context!=null) {
                            ...
                            LogUtil.log(TAG,"#unexcepted - context was null");
                        }
                    }else{
                        LogUtil.log(TAG,"#unexcepted - msg obj was incorrect");
                    }
                    break;
            }
        }
    ...
    public void handleAction(Context context,int sequence, TagAliasBean tagAliasBean){
        ...
        if(tagAliasBean == null){
            LogUtil.log(TAG,"tagAliasBean was null");
            return;
        }
        ...
        if(tagAliasBean.isAliasAction){
            switch (tagAliasBean.action){
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    LogUtil.log(TAG,"unsupport alias action type");
                    return;
            }
        }else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    LogUtil.log(TAG,"unsupport tag action type");
                    return;
            }
        }
    }
    private boolean RetryActionIfNeeded(int errorCode,TagAliasBean tagAliasBean){
        if(!ExampleUtil.isConnected(context)){
            LogUtil.log(TAG,"no network");
            return false;
        }
        //返回的错误码为6002 超时,6014 服务器繁忙,都建议延迟重试
        if(errorCode == 6002 || errorCode == 6014){
            LogUtil.log(TAG,"need retry");
            if(tagAliasBean!=null){
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        LogUtil.log(TAG,"action - onTagOperatorResult, sequence:"+sequence+",tags:"+jPushMessage.getTags());
        LogUtil.log(TAG,"tags size:"+jPushMessage.getTags().size());
        init(context);
        ...
        if(jPushMessage.getErrorCode() == 0){
            LogUtil.log(TAG,"action - modify tag Success,sequence:"+sequence);
            tagAliasActionCache.remove(sequence);
            String logs = getActionStr(tagAliasBean.action)+" tags success";
            LogUtil.log(TAG,logs);
            ExampleUtil.showToast(logs, context);
        }else{
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
        LogUtil.log(TAG,"action - onCheckTagOperatorResult, sequence:"+sequence+",checktag:"+jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            LogUtil.log(TAG,"tagBean:"+tagAliasBean);
            tagAliasActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogUtil.log(TAG,"action - onAliasOperatorResult, sequence:"+sequence+",alias:"+jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            LogUtil.log(TAG,"action - modify alias Success,sequence:"+sequence);
            tagAliasActionCache.remove(sequence);
